...
func (s *CanvasSuite) TestNewCanvas(c *C) {
	canvas, err := NewCanvas(5, 42, &DefaultConfig)
	c.Assert(err, IsNil)
	c.Assert(canvas, NotNil)
	c.Assert(canvas.w, Equals, 5)
	c.Assert(canvas.h, Equals, 42)

	_, ok := canvas.m.(*WS281x)
	c.Assert(ok, Equals, true)
}

func (s *CanvasSuite) TestInitialize(c *C) {
	m := NewMatrixMock()
	canvas := &Canvas{m: m}
	canvas.Initialize()

	c.Assert(m.called["Initialize"], Equals, true)
}
